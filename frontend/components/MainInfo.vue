<template>
  <div>
    <h2>Välkommen kollega på Chalmers bibliotek!</h2>
    <p>Här kan du rensa reservationshyllan, skapa framplockningslistor, byta PIN-kod och skapa nya bibliotekskonton.</p>
    <br>
    <h3>Lägg diskappen som en app på din dator</h3>
    <p>Sök upp https://foliodiskapp.lib.chalmers.se/</p>
    <p>I URL-fältet finns en app-ikon längst till höger. Klicka på den så installeras appen och du kan lägga den i verktygsfältet på datorn.</p>
    <p>I Chrome liknar den en dator med pil nedåt. Edge-ikonen visas nedan.</p>
    <img src="/img/Install-web-app-icon-edge.png">
    <h3>Använda diskappen på mobilen</h3>
    <p>Sök upp https://foliodiskapp.lib.chalmers.se/</p>
    <p>Lägg länken på startskärmen. </p>
    <p>Iphone: Klicka på "Dela länk" i verktygsfältet och sedan "Lägg till på hemskärmen".</p>
  </div>
</template>
